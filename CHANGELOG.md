...
- **tutti-graph: events as ports, the graph side** (design doc 013,
  ...
  | a writer past its capacity refused the push and the executor counted it | the same, at the node's declared capacity: **drop the newest, and count it** (`EventRejected::Full`, `Executor::dropped_events`). The verifier now checks that no slot downstream of a writer can refuse: merges hold the sum of their inputs, delays their source's rate |
  ...
  | an event delay FIFO retuned shorter (or to a lower rate) could count what it already held against its new limit and refuse new events | the new limit adds what is queued |
