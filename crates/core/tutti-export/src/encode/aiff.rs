...
//!
//! The IFF writer already loops over `num_channels` planes and stamps the count
//! into the COMM chunk, so it carries any width — it just needs the per-channel
//! `planes` [`rechannel`](crate::encode::rechannel) produced.
...
pub(crate) fn encode(planes: &[Vec<f32>], request: &EncodeRequest<'_>) -> Result<()> {
    ...
    // `planes` is already the per-channel planar shape the IFF writer wants.
    let planes: Vec<&[f32]> = planes.iter().map(|c| c.as_slice()).collect();
