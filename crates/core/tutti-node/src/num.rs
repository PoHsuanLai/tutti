//! The numeric tower the node contract is generic over: the [`Sample`] SIMD
//! geometry, the [`Num`] / [`Int`] / [`Float`] / [`Real`] arithmetic ladder,
//! and the block-size constants every buffer is shaped by.
//!
//! # Why the ladder and not `num-traits`
//!
//! Every rung is implemented for SIMD vectors as well as scalars ([`Num`] for
//! `f32x8`, [`Float`] for `f64x4`), which is what lets one DSP expression
//! compile to either a per-sample or a per-block form. A general numeric crate
//! abstracts over scalars; this abstracts over *lanes*, and the two are not the
//! same interface — `reduce_add`, `from_frame` and `to_frame` have no scalar
//! meaning, and `Float::to_f64` deliberately answers `0.0` for a vector because
//! there is no single value it could answer with.
//!
//! Relocated wholesale from the fork. The arithmetic is untouched; only the
