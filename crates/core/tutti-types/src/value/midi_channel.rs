...
//!
//! `tutti-midi-types` has a `pub type MidiChannel = u8` alias, which prevents
//! none of that — an alias is the same type. This is the real one. It lives
//! here rather than beside the alias because a document has to persist it, and
//! `tutti-midi-types` carries no serde.
//!
