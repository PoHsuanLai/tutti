...
/// A block of planar scratch, [`MAX_SAMPLER_CHANNELS`] lanes wide (2 KiB at 64 frames).
///
/// Owned by the node that renders through it, built once on the control
/// thread (boxed, so a node that moves does not move it all). Carries
...
/// What a voice's block read renders through: the lanes it hands on, the
/// lanes its stretch filter reads from, and the block's positions and taps.
/// One per rendering node (a pool reads every slot through its one), about
/// 7 KiB at 64 frames, built on the control thread, so a voice's block read touches no
/// stack arrays of its own.
#[derive(Debug)]
pub(crate) struct BlockScratch {
    ...
    /// `out`.
    pub(crate) raw: Lanes,
    /// Where each frame of the block reads (`None`: silence).
    pub(crate) positions: Box<[Option<SamplePosition>; LANE_FRAMES]>,
    /// The memory tier's along-time read (`MemorySource::read_placed_lanes`).
    pub(crate) gather: Box<Gather>,
}
...
impl BlockScratch {
    ...
    pub(crate) fn new() -> Self {
        Self {
            out: Lanes::new(),
            raw: Lanes::new(),
            positions: Box::new([None; LANE_FRAMES]),
            gather: Box::new(Gather {
                taps: [[0; 4]; LANE_FRAMES],
                frac: [0.0; LANE_FRAMES],
                live: [false; LANE_FRAMES],
                y: [[0.0; LANE_FRAMES]; 4],
            }),
        }
    }
...
/// Each frame's four taps and fraction, and the taps gathered into four
/// lanes, for the cubic along time. Written in full for the frames a block
/// reads before any is used.
#[derive(Debug)]
pub(crate) struct Gather {
    pub(crate) taps: [[usize; 4]; LANE_FRAMES],
    pub(crate) frac: [f32; LANE_FRAMES],
    /// Whether frame `i` has a sample (a position inside the file).
    pub(crate) live: [bool; LANE_FRAMES],
    pub(crate) y: [Lane; 4],
}
