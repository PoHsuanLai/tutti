...
pub trait PluginParameters: Sync {
    ...
    ///
    /// [`get_preset_data`](Self::get_preset_data)'s empty `Vec` conflates
    /// "nothing saved" with "`effGetChunk` returned an error", and a host that
    /// cannot separate them downgrades the second to a parameter snapshot,
    /// losing the plugin's non-parameter state.
    ///
    /// The default never errs, which is right for a plugin implementation: it is
    /// the source of the bytes and has no dispatch result to relay. The host's
    ...
    /// return `1` on success, and a plugin handed a truncated blob, a foreign
    /// one, or a format version it no longer reads is expected to reject it.
    /// This used to be `()`, so a host could not tell a refusal from a restore.
    ///
