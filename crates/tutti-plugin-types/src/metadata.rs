...
//!
//! Shared value vocabulary across the host crates and the `tutti-plugin`
//! IPC protocol (where it rides on `BridgeMessage::PluginLoaded`). The
//! `Serialize`/`Deserialize` derives are gated behind the `serde` feature,
//! which `tutti-plugin` enables for its bincode wire path.
...
/// [`PluginInfo::buses`]) is ordered per direction by bus index; bus 0 of each
/// direction is the main bus and later input buses are aux/sidechain inputs.
///
