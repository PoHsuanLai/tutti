...
| # | Finding | Site | Status |
|---|---|---|---|
| RT-1 | Two `crossbeam_channel::unbounded()` channels are fed from `audioMasterProcessEvents` and `automate`, which VST2 plugins call from inside `processReplacing` — i.e. the audio thread. `try_send` on an unbounded channel cannot fail, so `let _ =` guards nothing and each send allocates; the queue also grows without bound if the receiver lags. **Verified by direct reading.** Fix: bounded `ArrayQueue`, the idiom already used in `tutti-plugin`'s `ipc_client/audio/channels.rs`. | `tutti-vst2-host/src/instance.rs:157-158`, `src/host.rs:132,145` | open |
| RT-2 | `audio_pipeline.rs` documents the per-block path as allocation-free, guarded by `process_is_alloc_free` — which drives the `NanPlugin` stub, so no real loader is exercised. The real path allocates every block: `collect()` + `clone()` discard the borrowing, zero-alloc `ProcessOutputRef` that `tutti-vst3-host` returns deliberately (its owning conversion is marked "Allocates; off-RT only"). **Verified by direct reading.** Fix: propagate the borrowing shape through `PluginInstance::process`, and extend the test to a real loader. | `tutti-plugin-server/src/audio_pipeline.rs:207,966`; `src/loaders/vst3.rs:490-491`; also `loaders/vst2.rs:217,235` (SmallVec spill), `vst3.rs:748`/`clap.rs:458` (`to_string` on an error path) | open |
| RT-3 | `push_transport_request` takes a blocking `Mutex` and `Vec::push`es, reached from `clap_host_transport_control`; CLAP permits `request_*` from the audio thread. The same file uses non-blocking `let Ok(guard) = … else` elsewhere. Reported at lower confidence — **not yet independently verified**; confirm the audio-thread path before fixing. | `tutti-clap-host/src/host/callbacks.rs:635`, state at `src/host/state.rs:320`; cf. `:528,:860` | open |
| RT-4 | `Recorder` has `stop()` but no `Drop`; `sink.finalize()` runs only inside the spawned closure after `running` clears, which only `stop()` does. Dropping without `stop()` detaches the thread and leaves the WAV header unpatched — the file is unreadable. Every sibling thread owner joins in `Drop`. **Not yet independently verified.** | `tutti-io/src/recorder.rs:99-178` | open |
| RT-5 | A documented RT guarantee names a gating test that does not exist — the crate has no `tests/` directory. Either write the test or drop the claim. | `tutti-cpal/src/lib.rs:76-77`, `src/output.rs:104` | open |
...
| # | Finding | Site | Status |
|---|---|---|---|
| SR-1 | 14 constructors build at a placeholder rate and rely on a later `set_sample_rate`; not one documents that `process()` before it yields silently wrong-rate audio (8.8% error at 48 kHz). `bevy-tutti` documents the identical hazard in five lines. **Minimum fix:** document all 14. **Real fix:** make the rate a constructor argument (SR-2). | `tutti-units`: `delay.rs`, `svf.rs`, `ladder.rs`, `phaser.rs`, `limiter.rs`, `lfo.rs`, `convolution/node.rs`, `modulated_delay.rs`, `compressor.rs`, `gate.rs` | open |
| SR-2 | Five idioms for one need: mandatory validated Config field (`tutti-analysis`, with its reasoning written out), mandatory ctor arg (`tutti-spatial`, `tutti-sampler`), Config field + setter (`tutti-polysynth`), placeholder + setter (`tutti-units`), context resource (`bevy-tutti`). Target: constructor argument. | workspace-wide | open |
...
| SR-4 | `DEFAULT_SR` (raw `f64`) and `DEFAULT_SAMPLE_RATE` (newtype) are used interchangeably in sibling files. | `tutti-units/src/compressor.rs:5`, `src/gate.rs:5` | open |
...
| # | Finding | Site | Status |
|---|---|---|---|
| D-1 | **Structural fix, highest leverage in the campaign.** The two crates using `#![doc = include_str!("../README.md")]` are the only two with zero README drift; adopting it workspace-wide would have prevented 19 of the 25 drift items found. | all crates; pattern at `tutti-midi-file/src/lib.rs:25`, `tutti-midi-hardware/src/lib.rs:87` | open |
| D-2 | The root README describes an engine that was never built: `TuttiEngine`, `TuttiGraph`, `TuttiNet`, `TransportManager`, `MidiSystem`, `chain!`/`mix!`/`stack!`, `params!`, `pipe_all`, `load_vst3`, `graph_mut` — **zero occurrences workspace-wide**. Every code block in the file is unbacked. Distinct from the planned facade crate: these are API shapes, not a missing package. | `crates/tutti/README.md` | open |
| D-3 | Nine further wholly-fictional README quick starts. | `tutti-core` (`TuttiNet`/`TransportManager`/`MeteringManager`/`PdcManager`), `tutti-sampler` (`Sampler::builder`), `tutti-analysis` (`compute_summary`/`TransientDetector`/`CorrelationMeter`; `PitchDetector` is `pub(crate)`), `tutti-export` (`Export` builder, `MidiTrack`, omits the mandatory clock), `tutti-plugin` (async `PluginClient::init`/`load_plugin`; the real ctor is 3-arg sync), `tutti-clap-host` (wrong crate name; `ClapInstance` belongs to another crate), `tutti-vst3-host` (`Vst3MidiEvent`), `bevy-tutti` (five `TuttiPlugin` builder methods, `SynthNode`, `MidiUnit`) | open |
| D-4 | The root README lists 7 crates against ~25 members, omitting `tutti-types` (the shared floor) and `tutti-cpal` (the only path to a sound card). Its one-line crate roles contradict each crate's own header: MIDI routing attributed to `tutti-core`, MPE/CC to `tutti-midi-hardware`, recording to `tutti-sampler`, spatial to `tutti-units`. | `crates/tutti/README.md` | open |
| D-5 | "Umbrella" names `tutti` upstairs and `bevy-tutti` downstairs — the two levels disagree on the entry point, the most basic fact a newcomer needs. | `crates/tutti/README.md:25` vs `tutti-core/src/lib.rs:8` | open |
| D-6 | Four README feature claims contradict the manifests — three of them assert exactly what a Cargo.toml comment was written to deny. | `bevy-tutti/README.md:342` (`dsp` feature), `:337` (soundfont implies synth), `tutti-export/README.md:121` (`midi`), `tutti-core/README.md:24` (`std`) | open |
| D-7 | `bevy-tutti`'s README says there is no ECS surface for export; the crate ships `ExportPlugin` and its own module doc opens "An export is an **entity**". The signature given is also wrong (4 args, `net` by value). | `bevy-tutti/README.md:277-285` vs `src/export/mod.rs:10,123` | open |
| D-8 | Bevy compatibility table says 0.17; the workspace pins 0.19. Highest-traffic line in a Bevy plugin README. | `bevy-tutti/README.md:362` | open |
| D-9 | Stale type names in rustdoc: `PluginHost::load` (no such type), three references to `ClapInstance` in a crate that has `ClapLoaded`/`ClapActive` (the name belongs to a different crate), `ClapInstance::process`. | `tutti-plugin-server/src/lib.rs:262`; `tutti-clap-host/src/instance/extensions.rs:3`, `instance/config.rs:62`, `host/mod.rs:51`; `tutti-plugin-types/src/channels.rs:43` | open |
...
| V-2 | `with_*` means both an alternate constructor (51 sites) and a chainable builder (68 sites), sometimes in one `impl` block 100 lines apart. Chainable wins; alternate constructors get real names. | workspace-wide; e.g. `tutti-units/src/limiter.rs:183` vs `:290` | open |
...
| V-6 | `AudioUnit` impls carry three different suffixes with no rule separating them, and the split is *within* one crate: `LimiterNode` beside `BrickwallLimiter`, `DistortionNode` beside `Compressor` and `Gate`, `DelayLineNode` beside `DownmixUnit`/`ChannelSumUnit`/`BusStripUnit`. 14 `*Node` vs 8 `*Unit` vs 6 bare. The crate's own doc header already states the rule its exports break: "Every node here is an `AudioUnit`". `*Unit` also re-collides with the `tutti-units`/`tutti-types` trap (V-5). Winner: `*Node`. | `tutti-units/src/lib.rs:3,119-169`; `tutti-soundfont`; `tutti-sampler/src/stretch` | open |
...
| V-11 | The block-length parameter is `block_size` at ~34 sites and `frames` at ~11 — and the split runs *within* `tutti-core`: `beat_window.rs` says `block_size`, `metering/rt.rs` says `frames`, for the same quantity. Capacity-vs-count is already carried by the `max_` prefix, not by this word. Winner: `block_size`. | `tutti-core/src/metering/rt.rs:35,55,86`, `metering/tap.rs:185` vs `transport/beat_window.rs:91,179,233` | open |
...
| V-5 | `tutti-units` (DSP nodes) and `tutti-types` (Hz/Db/Bpm measurement units) collide on "units", meaning opposite things. `tutti-core` is a vocabulary crate rather than a main crate; `tutti-cpal` names its dependency rather than its job. Rename is real churn — sequence it after the prose is true. | crate names | open |
