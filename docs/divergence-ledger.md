...
| D-10 | **DONE (in the documentation wave).** `tutti-types` now says 'Eight families' and lists exactly eight; `tutti-mod`'s module map gained the four modules it omitted. Verified by counting. Self-miscounts: `tutti-types` says "Four families" then lists six (11 modules exist); `tutti-mod`'s module map omits four modules, one of which its own prose discusses at length. | `tutti-types/src/lib.rs:4`; `tutti-mod/src/lib.rs:170-176` | done |
...
| P-8 | **DONE as documentation, not an API change.** `tutti-soundfont`'s MIDI inbox is now documented as the one intended path, with `note_on`/`note_off` marked 'not the intended path' on the methods themselves and the reasons given (no `frame_offset`, unreachable by `MidiBus`, `&mut self` so unreachable once in a `Net`). Removing them is a breaking change with no consumer asking for it. `tutti-soundfont` offers two ways to start a note — the shared MIDI inbox and public MIDI-1 scalars — where `tutti-polysynth` offers one. Hide the rustysynth-shaped scalars. | `tutti-soundfont/src/lib.rs:152,187-195` | done |
...
| # | Finding | Site | Status |
|---|---|---|---|
| V-1 | **PARTIALLY DONE — the decode half only.** `MidiEvent::group()` now returns `MidiGroup` and `MidiMessage::channel()` returns `Option<MidiChannel>`, so the newtype survives decode, and `dawai-model` was updated to match (its `note_on_event` takes `MidiChannel`, and one now-redundant `MidiChannel::new` wrap is gone). **51 `channel: u8` sites remain**, and they are not all principled: `tutti-polysynth`'s voice allocator (21, incl. `VoiceSlot.channel` and `release(id, channel)`) and `tutti-midi-types`'s `mpe/zone.rs` public API (`master_channel`, `single_channel`, `pitch_bend_range_for`) are genuinely unfinished; the rest are test helpers and wire/FFI structs where `u8` is correct. Both workspaces compile and all tests pass, so this is a *stable* intermediate state rather than a broken one — but it is two vocabularies where the goal is one. Finish bottom-up: `mpe/zone.rs` public API, then `tutti-midi-runtime`, then the polysynth allocator. | `tutti-polysynth/src/voice.rs`; `tutti-midi-types/src/mpe/zone.rs`; ~51 sites | partial |
...
| V-4 | **DONE.** The bare `Plugins::flush` -> `save`. Its doc already said it commits the catalog to its backing store; the other three flushes drain a buffer onward (sampler ring, CLAP param queue, writer) while this one writes a file that outlives the process. No external callers. `flush` names four unrelated operations across four crates; the bare `Plugins::flush` (which means "persist to disk") is the one that reads as the wrong verb. | `tutti-mod/src/param.rs:115`; `tutti-sampler/src/voice/node.rs:177`; `tutti-plugin/src/host/plugins.rs:377`; `tutti-clap-host/src/instance/params.rs:286` | done |
...
| V-7 | ~~`Plugin::open` is the one place `open` does not mean an OS resource~~ - **MISDIAGNOSED.** It does. `Plugin::open(path, sample_rate)` takes a filesystem path and loads a dynamic library through it - the same sense as `MicIn::open`, `Tap::open` and the ALSA seq handle. This entry misread a `Path` argument as a non-resource. | - | wontfix |
...
| V-9 | **DONE.** `MpeModeHandle` -> `MpeModeRes`, matching the 18 Resources that already carry the suffix and its own three-lines-away sibling `MpeModeConfig`. Four sites. 18 Bevy `Resource`s carry `*Res`, 8 do not — including `MpeModeHandle`, a Resource wearing a Handle noun three lines from `MpeModeConfig` (also a Resource) in the same file as `MidiBusRes` (which has the suffix). `AudioConfig` is both unsuffixed and collides with an unrelated `AudioConfig` in `tutti-plugin`. **The other seven proposed renames are DECLINED — the split is a rule, not drift.** Recounted: bevy-tutti has **41** Resources, 21 suffixed and 20 not — an even split, not "18 of 26". Reading all 41, the suffix tracks one property exactly: every `*Res` is a **newtype wrapping a foreign engine type**, and the suffix is what keeps the Bevy resource from colliding with the thing it holds — `AudioGraphRes(Net)`, `TransportRes(Transport)`, `MeteringRes(MasterMeter)`, `MetronomeRes(Arc<ClickState>)`, `MidiBusRes(MidiBus)`, `MidiRoutingRes(MidiRoutingTable)`, `MidiOutSinkRes(Arc<MidiOutSink>)`, `PluginsRes(Plugins)`, `ModBusRes(Arc<ModBus>)`, `MidiIoRes(MidiSession)`, `DiskStreamerRes(DiskStreamer)`, `AudioTapRes(AudioTap)`, `MpeModeRes(MpeModeRequest)`. Every unsuffixed one is a struct bevy-tutti **authored itself**, with a name no engine type claims: `AudioConfig`, `EngineNodes`, `MidiTargetRegistry`, `InstalledMidiSources`, `MidiOutDrops`, `MidiRouteFallback`, `GraphDirty`, `MasterSources`, `GraphLatency`, `ModulationMatrix`. `graph/resources.rs`'s own header states it ("Each other subsystem keeps its `*Res` beside its own module") three lines above `AudioConfig` and `AudioGraphRes` **in the same file** — the author put the unsuffixed and the suffixed side by side deliberately. Suffixing the other seven would spend the suffix's only job. The claimed `AudioConfig` collision with `tutti-plugin`'s is also not one: bevy-tutti never imports that crate's, and the two live in different workspaces' dependency cones. `MpeModeHandle` was the one real case — a Resource wrapping `MpeModeRequest` and therefore owed the suffix — and it is fixed above. | `bevy-tutti/src/midi/endpoint/bus.rs:56,78`, `graph/resources.rs:16`, `graph/transport.rs:157`, `midi/endpoint/target.rs:88`, `midi/sequence.rs:85`, `midi/hardware/hardware_out.rs:198`, `midi/inbound/route.rs:135`; collision at `tutti-plugin/src/util/config/catalog.rs:90` | done |
| V-10 | **DONE.** `BlockInput::fill` -> `refill`, with its five impls (`param_automation_source`, `harmony_source`, `transport_source`, `note_expression_source`, and the test double in `input_slot`) and the inherent `fill`s the trait arms delegate to. All `pub(crate)`/`pub(super)`, so nothing outside the crate moved. `refill` is the author's own word for it: `param_automation_source` already had `refill_reuses_queue_storage_without_reallocating` and a "no extra queue pushed on refill" assertion before this rename, because the payload is scratch **reused across blocks** — each impl clears `out` and repopulates it, which is a refill and not a fill. Ordinary English "fills" in prose was left alone; only sites naming the method moved. Original finding: `fill` means two things: write audio into an output plane (`tutti-export`, matching `slice::fill`) vs populate a control-side struct with per-block parameter/transport/harmony changes (`tutti-plugin`). | `tutti-export/src/render/driver.rs:172,186,235` vs `tutti-plugin/src/host/node/input_slot.rs:60` + 5 impls | done |
...
| V-14 | ~~"frame" means two things in `bevy-tutti` prose~~ — **MISDIAGNOSED; do not "fix".** The entry's premise is that no *item* is named `Frame`, making this prose-only. Two are, and both are public API in **`tutti-mod`, a Bevy-free crate**: `ModPreFrame` (the once-per-frame modulation producer) and `ModDelivery::PerFrame`. `ModDelivery` is a deliberately designed three-point axis — `PerFrame` / `PerBlock` / `PerSample` — whose own doc says "the names say what actually differs — the **rate**", and every tier documents falling back *to* `PerFrame`. "Frame" there is one of three named rates, not a loose synonym, so the ECS-tick sense is load-bearing vocabulary rather than drift. The count is also off by a factor of six: ~180 occurrences across ~30 files, not ~30, and they are internally consistent throughout (`per-frame commit`, `retried next frame`, `frame thread`, `frames_since_snapshot`). Rewriting them to "tick" would leave the prose unable to name the `PerFrame` variant it explains. The audio sense is confined to `graph/pump.rs` and the `io`/sampler edges, where it is always spelled as a *count* (`frames`, `frame_offset`, `0-frame poll`) rather than as a unit of time — the two senses are already distinguished by that, and `pump.rs:27` states the rule outright: "is a frame count, never a sample count". | — | wontfix |
...
| L-3 | **MISDIAGNOSED - see df6c566f.** vst3's `types/` is 4201 lines against clap's 1207: 3.5x LARGER, not smaller. The entry has the comparison backwards, so there is no inverted split rule to fix. ~~The `types/` split rule is inverted~~ — **MISDIAGNOSED; do not "fix".** The audit claimed `tutti-clap-host`'s single 1207-line `types.rs` is larger than the whole of `tutti-vst3-host`'s `types/` directory. Measured: vst3's directory is **4201 lines** across five files (`events.rs` alone is 2373), three and a half times clap's file. The rule is not inverted — vst3 split because it is genuinely much bigger, and clap (1207), au (760) and vst2 (115) are each a reasonable single file. No action. | — | wontfix |
