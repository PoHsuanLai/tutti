...
| # | Finding | Site | Status |
|---|---|---|---|
| V-1 | **SIZED, deferred to its own pass.** MIDI newtypes die on decode: constructors take `MidiGroup`/`MidiChannel`, but `MidiEvent::group()` returns `u8`, `MidiMessage::channel()` returns `Option<u8>`, `SmfTimedEvent.channel` is `u8` — with no stated rationale at any of the three, while `tutti-types` mints both newtypes. Consumers then carry bare `u8` downstream. **Measured blast radius (the audit declined to size it): 82 call sites binding from those accessors, plus 70 `channel: u8` fields — ~152 sites across the engine and adapter, spanning every MIDI crate and their DSP consumers.** This is a single coherent change and must not be attempted alongside other work: a half-migration leaves two vocabularies where there is currently one honest one. Do it as a dedicated pass, bottom-up (`tutti-types` accessors first, then midi-types, midi-file, midi-runtime, then DSP consumers), with the app workspace fixed in the same change. | `tutti-midi-types/src/ump/mod.rs:84`, `src/message.rs:377`; `tutti-midi-file/src/smf.rs:49`; ~152 consumer sites | deferred |
